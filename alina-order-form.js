(function (d) {
  ...
  b.onload = function () {
    var inputPhone = d.querySelector('#inputPhone')
    var inputName = d.querySelector('#inputName')
    var submitButton = d.querySelector('#submitButton')
    var alert = d.querySelector('#alina-form-order-alert')
    ...
    function validatePhone (phone) {
      ...
      var dialCode
      var cellularCode
      var validPhoneByCountryCode
      var validPhoneByCellularCode
    
      listOfCountries.some(country => {
        if (phone.includes(country.dial_code)) {
          var phoneSplitA = phone.split(country.dial_code)
    ...
    inputPhone.onkeyup = function (e) {
      ...
      inputPhone.style['border-color'] = '#BDBDBD'
      alert.innerHTML = ''
    }

    inputName.onkeyup = function (e) {
      var value = e.target.value.replace(/[^\w\s]/gi, '').slice(0, 32)
      if (value) {
      ...
      inputName.style['border-color'] = '#BDBDBD'
      alert.innerHTML = ''
    }

    submitButton.onclick = function () {
      var phone = validatePhone(inputPhone.value)
      var validPhone = ''
      if (phone.validPhone) {
      ...
      if (!validPhone) {
        alert.innerHTML = '<strong>Mohon isi nomor whatsapp yang valid!</strong>'
        inputPhone.style.background = '#FFCDD2'
        inputPhone.style['border-color'] = '#F44336'
      } else {
        if (!inputName.value) {
          alert.innerHTML = '<strong>Mohon isi nama Anda!</strong>'
          inputName.style.background = '#FFCDD2'
          inputName.style['border-color'] = '#F44336'
        } else {
          inputPhone.style.background = '#FFFFFF'
          inputPhone.style['border-color'] = '#BDBDBD'
          ...
          alert.innerHTML = 'Terimakasih <strong>' + inputName.value + '</strong>!<br />Tim kami akan segera menghubungi Anda di nomor <strong>' + validPhone + '</strong>.'
          alert.classList.add('green')

          var data = 'name=' + inputName.value + '&phone=' + inputPhone.value
          ...
          xhttp.onreadystatechange = function() {
            if (this.readyState == 4 && (this.status >= 200 && this.status < 500)) {
              ...
              window.open('https://puspanegara.com/formulir/', '_self')
            }
          }
          // xhttp.open('POST', 'https://alinagroup.agency/api/lead', true)
          xhttp.open('POST', 'https://api/puspanegara.com/lead', true)
